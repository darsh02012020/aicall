...
def universal_architect_optimizer(code):
    ...
    # 1. ARCHITECT IMPORTS (Ensuring math and concurrent libs)
    ...
    # 4. CRITICAL: DUPLICATE BLOCK CLEANER
    # Ye purane stream-based groupingBy aur summingDouble blocks ko nikaal dega
    old_logic_patterns = [
        rf'Map<.*?>\s+\w+\s*=\s*{list_name}\.stream\(\).*?\.collect\(.*?\);',
        rf'//\s*\d+️⃣.*?\n', # Removes old numbering like 1️⃣, 2️⃣
        rf'{list_name}\.(?:parallelStream|stream).*?\.collect\(.*?\);'
    ]
    ...
    for m_name in used_maps:
        # Avoid Collision with local variables like 'topUsers' result list
        ...
        if not best_key:
            best_key = next((f for f in id_fields if ("product" in f.lower() and is_counter)), 
        ...
        # Patch Sorting/Max types
        if final_v_type == "BigDecimal":
            code = re.sub(rf'\.sorted\(.*?Double\.compare.*?\)', f'.sorted(Map.Entry.<String, BigDecimal>comparingByValue().reversed())', code)
            merge_logics.append(f"{actual_m_name}.merge({item_name}.{best_key}, val, BigDecimal::add);")
        else:
            code = re.sub(rf'\.max\(Map\.Entry\.comparingByValue\(\)\)', f'.max(Comparator.comparingLong(e -> e.getValue().sum()))', code)
    ...
    tips.append("🚀 <b>Architect Clean-Sweep:</b> Purane duplicate blocks delete kar diye gaye hain aur types sync kar diye hain.")
    return code, tips
